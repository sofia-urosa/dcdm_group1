...
OUTPUT_DIR <- "/Users/sof/Documents/Msc/DCDM/cw1/dcdm_group1/outputs" #to save data!
...
```{r}
#save clean data to insert to sql
...
```{r}
summary(sop_compliant_data)
malformed_gene_id <- sop_compliant_data %>% 
...
```{r}
inner_join(sop_compliant_data, c_disease_info, by = "gene_accession_id")
```



```{r}
distinct_mouse_strain<-distinct(sop_compliant_data, mouse_life_stage)
print(distinct_mouse_strain)

distinct_mouse_strain<-distinct(data, mouse_life_stage)
print(distinct_mouse_strain)

c_disease_info[is.na(sop_compliant_data$mouse_life_stage), ]


sapply(c_procedure, function(x) sum(is.na(x)))
