...
#parse info through command line:
...
cat("Input directory:", opts$inputdir, "\n")
cat("File:", opts$file, "\n")
...
DATA_DIR <- "/Users/sof/Documents/Msc/DCDM/dcdm_group1"
OUTPUT_DIR <- "/Users/sof/Documents/Msc/DCDM/dcdm_group1/outputs" 
...
data <- read.csv(file.path(DATA_DIR, "/outputs/merged_all.csv"))
...
valid_mouse_stages <- c("E12.5", "E15.5", "E18.5", "E9.5", "Early adult", "Late adult", "Middle aged adult")
...
#check any df for compliance on sop 
  ...
  for (df_name in names(df_list)) {  #for each df...
    ...
    if (length(missing_col) > 0){
      ...
      qc[[length(qc) + 1]] <- data.frame(
        ...
        value = paste("No action needed. For informative purposes"),
        stringsAsFactors = FALSE
      )
    }
    ...
    #in case a df has no matching columns
    if (length(common_fields) == 0) {cleaned_list[[df_name]] <- df
    next
    } 
    ...
    if (is.null(bad_unique) || length(bad_unique) == 0) {
      cleaned_df <- df} 
    else {
      cleaned_df <- df[-bad_unique, , drop = FALSE]
    }
...
#now check for SOP compliance

orth_sop_compliant = validate_sop(list(params = c_params, procedure = c_procedure, disease_info = c_procedure),sop)
...
c_data <- data %>%
  ...
  mutate(parameter_id = toupper(parameter_id)) %>%
  mutate(mouse_life_stage = str_to_sentence(str_to_lower(mouse_life_stage))) %>%
  mutate(gene_symbol = str_to_sentence(str_to_lower(gene_symbol)))
...
sop_compliant_data <- c_data %>%
  ...
  { 
    qc_corrected_mouse_strain <<- .$qc                #store QC from check_chr
    df <- .$data                                      #corrected data
    result <- validate_sop(list(clean_data = df), sop)
    qc_sop_compliant <<- result$qc                    #store QC from validate_sop
    result$data$clean_data      #return
  }
...
```{r}
#save clean data
write.csv(sop_compliant_data, file = file.path(OUTPUT_DIR, "clean_data.csv"), row.names = FALSE)
write.csv(c_disease_info, file = file.path(OUTPUT_DIR, "clean_disease_info.csv"), row.names = FALSE)
write.csv(c_params, file = file.path(OUTPUT_DIR, "clean_params.csv"), row.names = FALSE)
write.csv(c_procedure, file = file.path(OUTPUT_DIR, "clean_procedure.csv"), row.names = FALSE)
```
